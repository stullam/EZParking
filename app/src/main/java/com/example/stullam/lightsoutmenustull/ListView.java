...
//    private double[][] SortedListByDistances = new double[10][2];
    ...
    public ArrayList<LatLng> parkSpots = new ArrayList<LatLng>();
    public ArrayList<ParkingSpot> distanceSortedSpots = new ArrayList<ParkingSpot>();
    public ArrayList<LatLng> availableSpots = new ArrayList<LatLng>();
...
//        parkSpots.add(Parking10);
    System.out.println(123);
        ...
        Cursor c = db.query(EZParkingContract.EZParking.PARKING_TABLE_NAME, projection, null,null,null,null,null);
        if(c != null && c.getCount() > 0) {
            ...
            while(!c.isLast()) {
                ...
                parkSpots.add(new LatLng(latitude,longitude));
                c.moveToNext();
            }
        }

        for(int i=0;i<parkSpots.size();i++){
            double dis = Math.sqrt((parkSpots.get(i).latitude - CurrentSpot[0])*
                                      (parkSpots.get(i).latitude - CurrentSpot[0])
                                       +(parkSpots.get(i).longitude - CurrentSpot[1])*
                                        (parkSpots.get(i).longitude - CurrentSpot[1]));
            if(dis<distance) {
                   availableSpots.add(parkSpots.get(i));
            }
        }
            ...
            spotButton.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    if (v.getId() == spotButton.getId()) {
                        ...
                        ImportantSpotArray[0] = availableSpots.get(finalI).latitude;
                        ImportantSpotArray[1] = availableSpots.get(finalI).longitude;
